// Bundles the widget lab into widget-bundle.js (IIFE, so a static page can load
// it over file://). apexcharts & leaflet are heavy, so we don't bundle them: the
// chart/map pages pull them from a CDN as globals (window.ApexCharts / window.L),
// and this build aliases the widgets' dynamic `import('apexcharts')` /
// `import('leaflet')` to those globals. Their stylesheets — imported as `?raw`
// and injected into the widget shadow roots — are inlined from node_modules here.
...
const labCdn: BunPlugin = {
  ...
  setup(build) {
    ...
    build.onLoad({ filter: /.*/, namespace: 'cdn-global' }, (a) => ({
      loader: 'js',
      // No throw at module top-level: this evaluates lazily on first use (a chart
      // or map instance), and only those pages load the CDN <script>.
...
async function bundle(): Promise<boolean> {
  const res = await Bun.build({ entrypoints: ['./entry.ts'], format: 'iife', plugins: [labCdn] });
  if (!res.success) {
    for (const log of res.logs) console.error(String(log));
    return false;
  }
  await Bun.write('./widget-bundle.js', res.outputs[0]!);
  console.log(`built widget-bundle.js · ${new Date().toLocaleTimeString()}`);
  return true;
}
...
if (process.argv.includes('--watch')) {
  ...
  for (const dir of ['.', '../../../shared/widgets']) {
